script.set_name('EXO Troll v1.0')
toast.add_info('Loaded & Ready!')

local sub0 = menu.player_root():add_submenu('Cages', {}, function() end)
local sub1 = menu.player_root():add_submenu('Spawn Attackers', {'Very agresive attackers.'}, function() end)
local sub2 = menu.script_root()
...
--[[sub1:add_button("Remove Attackers", {}, function()
    if not spawned_attackers then
        toast.add_info("No attackers to remove.")
        return
    end

    for i, ped in ipairs(spawned_attackers) do
        if native.entity.does_entity_exist(ped) then
            native.entity.set_entity_as_mission_entity(ped, true, true)
            native.entity.delete_entity(ped)
        end
    end
    toast.add_info("Attackers removed.")
end)--]]

-- Drunk Slap Toggle
sub2:add_toggle("Drunk Slap toggle", {"Smack em!"}, function(toggle)
